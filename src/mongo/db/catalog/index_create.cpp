...
Status MultiIndexBlock::insertAllDocumentsInCollection(std::set<RecordId>* dupsOut) {
    ...
    // Need the index build to hang before the progress meter is marked
    // as finished so we can reliably check that the index build
    // has actually started in js tests.
